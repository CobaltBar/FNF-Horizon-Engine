...
class ErrorState extends MusicState
{
	...
	public override function create():Void
	{
		...
		shouldBop = false;
		add(errorTitle = Util.createText(FlxG.width * .5, -100, 'Error Caught', 36, Path.font('vcr'), 0xFFFF0000, CENTER));
		errorTitle.screenCenter(X);
		add(errorDescription = Util.createText(0, FlxG.height + 100, @:privateAccess Log.log, 18, Path.font('vcr'), 0xFFFFFFFF, LEFT));
		add(Util.createText(0, FlxG.height - 50,
